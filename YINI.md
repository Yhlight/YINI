...
### YBIN: 高性能二进制格式
YBIN是YINI引入的一种高性能、完全序列化的二进制格式。它的设计目标是为了在生产环境中实现零解析、极速加载。YBIN文件将一个或多个.yini文件的所有配置信息（包括继承、宏展开和变量引用）完全解析和烘焙（Cook）成一个单一的二进制资产。

#### 主要优点
- **极致性能**: YBIN文件是内存映射的，加载速度极快，几乎没有CPU开销，因为它避免了文本解析、AST构建和语义分析。
- **单一文件部署**: 将多个.yini文件（包括所有`[#include]`的文件）合并成一个独立的.ybin文件，简化了游戏的资产分发。
- **安全性**: 二进制格式不易于手动编辑，可以防止最终用户意外修改配置。

#### 创建YBIN文件
YBIN文件通过YINI的命令行工具（CLI）创建。使用`cook`命令可以将指定的.yini文件编译成.ybin文件。

```bash
yini cook -o <output.ybin> <input1.yini> [<input2.yini> ...]
```
- `-o <output.ybin>`: 指定输出的.ybin文件名。
- `<input1.yini> ...`: 一个或多个输入的.yini文件。

CLI工具会解析所有输入文件，处理所有的指令（如继承和宏），并生成一个优化过的二进制文件。

#### YBIN内部格式
YBIN文件中的所有数值类型（包括整数和浮点数）都以64位浮点数（`double`）的形式存储。
