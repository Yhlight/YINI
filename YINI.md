...
## 继承
在配置块的方括号 `]` 之后，你可以使用 `:` 来继承一个或多个其他配置块。
...
### YBIN (二进制资产)
YINI 支持一种为性能而优化的“烘焙”二进制格式，称为 `.ybin`。与在运行时需要解析的 `.yini` 和 `.ymeta` 文件不同，`.ybin` 文件是一个完全解析、序列化的二进制资产，可以以零解析开销加载。这使其成为生产构建或需要最快加载时间的场景的理想选择。

`.ybin` 文件包含所有配置数据，包括继承和宏的解析结果，存储在一个高效的、平台无关的格式中。

#### 创建 `.ybin` 文件
你可以使用 YINI 命令行工具 (CLI) 的 `cook` 命令从一个或多个 `.yini` 文件创建一个 `.ybin` 文件。

```bash
yini cook -o <output.ybin> <input1.yini> [input2.yini]...
```

- `-o <output.ybin>`: 指定要创建的输出 `.ybin` 文件的路径。
- `<input1.yini>...`: 一个或多个输入 `.yini` 文件的列表。如果多个文件包含相同的键，后面的文件将覆盖前面文件的值。

当游戏或应用程序加载一个 `.ybin` 文件时，它会绕过整个解析和解析过程，直接将数据加载到内存中，从而显著提高启动速度。
