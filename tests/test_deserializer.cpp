...
TEST(JsonDeserializerTest, DeserializeDocument)
{
    ...
    auto name_it = std::find_if(section->pairs.begin(), section->pairs.end(), [](const auto& p){ return p.key == "name"; });
    ASSERT_NE(name_it, section->pairs.end());
    EXPECT_EQ(std::get<std::string>(name_it->value.data), "YINI");

    auto version_it = std::find_if(section->pairs.begin(), section->pairs.end(), [](const auto& p){ return p.key == "version"; });
    ASSERT_NE(version_it, section->pairs.end());
    EXPECT_EQ(std::get<double>(version_it->value.data), 1.0);

    auto enabled_it = std::find_if(section->pairs.begin(), section->pairs.end(), [](const auto& p){ return p.key == "enabled"; });
    ASSERT_NE(enabled_it, section->pairs.end());
    EXPECT_EQ(std::get<bool>(enabled_it->value.data), true);

    auto data_it = std::find_if(section->pairs.begin(), section->pairs.end(), [](const auto& p){ return p.key == "data"; });
