...
TEST(YmetaManagerTests, GetAndSetValue)
{
    ...
    EXPECT_EQ(std::any_cast<double>(ymeta_manager.get_value("my_key")), 123.0);
}
...
TEST(YmetaManagerTests, BackupMechanism)
{
    ...
    EXPECT_EQ(std::any_cast<double>(ymeta_manager.get_value("my_key")), 9.0);
}
...
TEST(YmetaManagerTests, SaveAndLoad)
{
    ...
    EXPECT_EQ(std::any_cast<double>(ymeta_manager_to_load.get_value("my_key")), 123.0);
    ASSERT_TRUE(ymeta_manager_to_load.has_value("my_string"));
    EXPECT_EQ(std::any_cast<std::string>(ymeta_manager_to_load.get_value("my_string")), "hello");
