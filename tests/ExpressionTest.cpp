...
std::unique_ptr<YINI::Expr> parseExpression(const std::string& source) {
    ...
    testTokens.push_back({YINI::TokenType::LEFT_BRACKET, "[", {}, 1, 1});
    testTokens.push_back({YINI::TokenType::IDENTIFIER, "dummy", {}, 1, 1});
    testTokens.push_back({YINI::TokenType::RIGHT_BRACKET, "]", {}, 1, 1});
    testTokens.push_back({YINI::TokenType::IDENTIFIER, "key", {}, 1, 1});
    testTokens.push_back({YINI::TokenType::EQUAL, "=", {}, 1, 1});
    testTokens.insert(testTokens.end(), tokens.begin(), tokens.end() - 1); // Exclude EOF
    testTokens.push_back({YINI::TokenType::END_OF_FILE, "", {}, 1, 1});
