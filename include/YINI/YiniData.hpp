...
namespace YINI
{
    ...
    using YiniVariant = std::variant<std::string, int, double, bool, std::unique_ptr<YiniArray>,
                                     std::unique_ptr<YiniMap>, std::unique_ptr<YiniDynaValue>,
                                     std::unique_ptr<YiniCoord>, std::unique_ptr<YiniColor>, std::unique_ptr<YiniPath>>;
    ...
    public:
        ...
        const MacroDefinition* getMacroDefinition(const std::string& key) const
        {
            auto it = defines.find(key);
            if (it != defines.end())
            {
    ...
    public:
        ...
        {
            for (const auto& [key, def] : other.defines)
            {
                this->addDefine(key, def.value, def.location.line, def.location.column);
            }
    ...
    private:
        ...
        std::map<std::string, MacroDefinition> defines;
    };
} // namespace YINI
