...
struct Array : public Expr {
    Array(std::vector<std::unique_ptr<Expr>> elements) : elements(std::move(elements)) {}
    YiniValue accept(ExprVisitor& visitor) const override { return visitor.visit(*this); }
...
struct Set : public Expr {
    Set(std::vector<std::unique_ptr<Expr>> elements) : elements(std::move(elements)) {}
    YiniValue accept(ExprVisitor& visitor) const override { return visitor.visit(*this); }
    std::vector<std::unique_ptr<Expr>> elements;
};
...
struct XRef : public Expr {
    XRef(Token section, Token key) : section(std::move(section)), key(std::move(key)) {}
    YiniValue accept(ExprVisitor& visitor) const override { return visitor.visit(*this); }
    Token section;
    Token key;
};
...
struct Schema : public Stmt {
    Schema(std::vector<std::unique_ptr<Section>> sections) : sections(std::move(sections)) {}
    void accept(StmtVisitor& visitor) const override { visitor.visit(*this); }
