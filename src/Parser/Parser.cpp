...
YINI::YiniValue apply_op(const YINI::YiniValue &left, const YINI::YiniValue &right, YINI::TokenType op)
{
    ...
    double left_d = std::holds_alternative<int>(left.data) ? static_cast<double>(std::get<int>(left.data))
                                                           : std::get<double>(left.data);
    double right_d = std::holds_alternative<int>(right.data) ? static_cast<double>(std::get<int>(right.data))
...
void Parser::parseSection()
{
    ...
    else if (is_include_section)
    {
        if (m_currentToken.type != TokenType::RightBracket)
        {
            throw YiniException("Expected ']' to close #include directive.", m_currentToken.line,
...
YiniValue Parser::parseValue()
{
    ...
    case TokenType::Identifier:
    {
        ...
        std::transform(id_val.begin(), id_val.end(), id_val.begin(), [](unsigned char c) { return std::tolower(c); });
        ...
        if (id_val == "dyna")
        {
            ...
            if (m_currentToken.type != TokenType::RightParen)
                throw YiniException("Expected ')' to close Dyna expression.", m_currentToken.line,
                                    m_currentToken.column);
            nextToken();
...
std::unique_ptr<YiniCoord> Parser::parseCoord()
{
    ...
    if (m_currentToken.type != TokenType::LeftParen)
        throw YiniException("Expected '(' after Coord.", m_currentToken.line, m_currentToken.column);
    nextToken(); // consume '('
    ...
    if (std::holds_alternative<int>(x_val.data))
        ...
        throw YiniException("Coord parameters must be numeric.", m_currentToken.line, m_currentToken.column);

    if (m_currentToken.type != TokenType::Comma)
        throw YiniException("Expected ',' in Coord.", m_currentToken.line, m_currentToken.column);
    nextToken();
    ...
    if (m_currentToken.type != TokenType::RightParen)
        throw YiniException("Expected ')' to close Coord expression.", m_currentToken.line, m_currentToken.column);
    nextToken();
    return coord;
}
...
std::unique_ptr<YiniColor> Parser::parseColor()
{
    ...
    if (m_currentToken.type != TokenType::LeftParen)
        throw YiniException("Expected '(' after Color.", m_currentToken.line, m_currentToken.column);
    nextToken(); // consume '('
    ...
    if (!std::holds_alternative<int>(r_val.data))
        throw YiniException("Color parameters must be integers.", m_currentToken.line, m_currentToken.column);
    color->r = std::get<int>(r_val.data);

    if (m_currentToken.type != TokenType::Comma)
        throw YiniException("Expected ',' in Color.", m_currentToken.line, m_currentToken.column);
    nextToken();
    ...
    if (m_currentToken.type != TokenType::RightParen)
        throw YiniException("Expected ')' to close Color expression.", m_currentToken.line, m_currentToken.column);
    nextToken();
    return color;
}
...
std::unique_ptr<YiniPath> Parser::parsePath()
{
    ...
    if (m_currentToken.type != TokenType::LeftParen)
        throw YiniException("Expected '(' after Path.", m_currentToken.line, m_currentToken.column);
    nextToken(); // consume '('
    ...
    if (m_currentToken.type != TokenType::RightParen)
        throw YiniException("Expected ')' to close Path expression.", m_currentToken.line, m_currentToken.column);
    nextToken();
    return path;
}
} // namespace YINI