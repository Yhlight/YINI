...
std::vector<std::string> Interpreter::get_macro_names() const { return m_globals.get_all_keys(); }

// Helper functions for type checking
static bool is_number(const YiniValue& value) { return std::holds_alternative<double>(value.m_value); }

static void check_number_operand(const Token& op, const YiniValue& operand) {
    if (!is_number(operand)) {
        throw RuntimeError("Operand must be a number for operator '" + op.lexeme + "'.", op.line, op.column,
                           op.filepath);
    }
}
...
static void check_number_operands(const Token& op, const YiniValue& left, const YiniValue& right) {
    if (!is_number(left) || !is_number(right)) {
        throw RuntimeError("Operands must be numbers for operator '" + op.lexeme + "'.", op.line, op.column,
                           op.filepath);
    }
}
...
void Interpreter::interpret(const std::vector<std::unique_ptr<Stmt>>& statements) {
    ...
    for (const auto& statement : statements) {
        if (auto* define = dynamic_cast<Define*>(statement.get())) {
            ...
            if (m_sections.count(section->name.lexeme)) {
                throw RuntimeError("Section '" + section->name.lexeme + "' has already been defined.",
                                   section->name.line, section->name.column, section->name.filepath);
            }
    ...
    for (const auto& section_pair : m_expression_map) {
        ...
        for (const auto& key_pair : section_pair.second) {
            ...
            visit(XRef(Token{TokenType::IDENTIFIER, section_name, {}, 0, 0, ""},
...
void Interpreter::build_expression_map(const Section* section) {
    ...
    if (m_resolving.count(section->name.lexeme)) {
        throw RuntimeError("Circular inheritance detected involving section '" + section->name.lexeme + "'.",
                           section->name.line, section->name.column, section->name.filepath);
    }
    ...
    for (const auto& parent_token : section->parents) {
        ...
        if (it == m_sections.end()) {
            throw RuntimeError("Parent section '" + parent_token.lexeme + "' not found.", parent_token.line,
                               parent_token.column, parent_token.filepath);
        }
...
YiniValue Interpreter::visit(const EnvVariable& expr) {
    ...
    throw RuntimeError(
        "Required environment variable '" + expr.name.lexeme + "' is not set and no default value is provided.",
        expr.name.line, expr.name.column, expr.name.filepath);
}
...
YiniValue Interpreter::visit(const XRef& expr) {
    ...
    if (m_currently_resolving_values.count(full_ref)) {
        throw RuntimeError("Circular reference detected for value '" + full_ref + "'.", expr.section.line,
                           expr.section.column, expr.section.filepath);
    }
...
YiniValue Interpreter::visit(const Map& expr) {
    ...
    for (const auto& pair : expr.pairs) {
        ...
        if (!std::holds_alternative<std::string>(key_val.m_value)) {
            throw RuntimeError("Map keys must evaluate to strings.", expr.brace.line, expr.brace.column,
                               expr.brace.filepath);
        }
...
YiniValue Interpreter::visit(const Call& expr) {
    ...
    if (!std::holds_alternative<std::string>(callee.m_value)) {
        throw YiniException("Can only call functions by name.", expr.paren.line, expr.paren.column,
                            expr.paren.filepath);
    }
    ...
    if (callee_name == "Dyna" || callee_name == "dyna") {
        if (expr.arguments.size() != 1) {
            throw YiniException("Dyna() expects exactly one argument.", expr.paren.line, expr.paren.column,
                                expr.paren.filepath);
        }
    ...
    if (callee_name == "Color" || callee_name == "color") {
        if (expr.arguments.size() != 3 && expr.arguments.size() != 4) {
            throw YiniException("Color() expects 3 (r, g, b) or 4 (r, g, b, a) arguments.", expr.paren.line,
                                expr.paren.column, expr.paren.filepath);
        }
        YiniMap color_map;
    ...
    if (callee_name == "Vec2" || callee_name == "vec2") {
        if (expr.arguments.size() != 2) {
            throw YiniException("Vec2() expects exactly 2 arguments (x, y).", expr.paren.line, expr.paren.column,
                                expr.paren.filepath);
        }
        YiniMap vec_map;
    ...
    if (callee_name == "Vec3" || callee_name == "vec3") {
        if (expr.arguments.size() != 3) {
            throw YiniException("Vec3() expects exactly 3 arguments (x, y, z).", expr.paren.line, expr.paren.column,
                                expr.paren.filepath);
        }
        YiniMap vec_map;
        vec_map["x"] = evaluate(*expr.arguments[0]);
    ...
    if (callee_name == "Vec4" || callee_name == "vec4") {
        if (expr.arguments.size() != 4) {
            throw YiniException("Vec4() expects exactly 4 arguments (x, y, z, w).", expr.paren.line, expr.paren.column,
                                expr.paren.filepath);
        }
        YiniMap vec_map;
        vec_map["x"] = evaluate(*expr.arguments[0]);
    ...
    throw YiniException("Unknown function call '" + callee_name + "'.", expr.paren.line, expr.paren.column,
                        expr.paren.filepath);
}
}  // namespace YINI