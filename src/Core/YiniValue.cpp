    ...
    std::visit(
        [&](const auto& v) {
            ...
            if constexpr (std::is_same_v<T, std::unique_ptr<YiniArray>>) {
                if (v)
                    m_value = std::make_unique<YiniArray>(*v);
                else
                    m_value = std::make_unique<YiniArray>();
            } else if constexpr (std::is_same_v<T, std::unique_ptr<YiniMap>>) {
                if (v)
                    m_value = std::make_unique<YiniMap>(*v);
                else
                    m_value = std::make_unique<YiniMap>();
            } else if constexpr (std::is_same_v<T, std::unique_ptr<DynaValue>>) {
                if (v)
                    m_value = std::make_unique<DynaValue>(*v);
                else
                    m_value = std::make_unique<DynaValue>(YiniValue{});
            } else {
                m_value = v;
            }
        },
