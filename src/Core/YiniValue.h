 ...
 */
using YiniValueBase = std::variant<std::monostate,  // Represents a null or uninitialized value
                                   bool, double, std::string, std::unique_ptr<YiniArray>, std::unique_ptr<YiniMap>,
                                   std::unique_ptr<DynaValue>>;

/**
 * @class YiniValue
 * @brief A wrapper around a std::variant to provide a type-safe container for any value that can be represented in
 * YINI.
 *
