...
class YbinData
{
  public:
    ...
    {
#ifdef _WIN32
        m_file_handle = CreateFileA(file_path.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
        ...
        if (LZ4_decompress_safe(base + m_header.data_table_offset, m_data_table_storage.data(),
                                m_header.data_table_compressed_size, m_header.data_table_uncompressed_size) < 0)
        ...
        if (LZ4_decompress_safe(base + m_header.string_table_offset, m_string_table_storage.data(),
                                m_header.string_table_compressed_size, m_header.string_table_uncompressed_size) < 0)
...
template <typename T> bool get_value(void *handle, const char *key, T *out_value)
{
    ...
    std::visit(
        [&](auto &&arg)
        {
            ...
            else if constexpr (std::is_same_v<T, double>)
            {
                if constexpr (std::is_same_v<V, int64_t>)
                {
                    *out_value = static_cast<double>(arg);
                    success = true;
                }
                if constexpr (std::is_same_v<V, double>)
                {
...
template <typename T> bool get_array_item(void *handle, const char *key, int index, T *out_value)
{
    if (!handle || !key || !out_value)
        return false;
    YINI::Config *config = static_cast<YINI::Config *>(handle);
...
template <typename T> bool get_map_value(void *handle, const char *key, const char *sub_key, T *out_value)
{
    if (!handle || !key || !sub_key || !out_value)
        return false;
    YINI::Config *config = static_cast<YINI::Config *>(handle);
    YINI::YiniVariant value = config->find(key);
...
template <typename T> bool get_struct_value(void *handle, const char *key, T *out_value)
{
    if (!handle || !key || !out_value)
        return false;
    YINI::Config *config = static_cast<YINI::Config *>(handle);
