...
extern "C"
{
        ...
        catch (const YINI::YiniException &e)
        {
            if (error_buffer && buffer_size > 0)
            {
                std::string error_msg =
                    "Error at [" + std::to_string(e.getLine()) + ":" + std::to_string(e.getColumn()) + "]: " + e.what();
    ...
    {
        if (!handle || index < 0 || index >= handle->doc.getSections().size())
            return nullptr;
    ...
    YINI_API void yini_set_double_value(YiniDocumentHandle *handle, const char *section_name, const char *key,
                                        double value)
    {
        if (!handle || !section_name || !key)
            return;
    ...
    YINI_API int yini_section_get_pair_key_by_index(const YiniSectionHandle *section_handle, int index, char *buffer,
                                                    int buffer_size)
    {
        if (!section_handle)
            return 0;
        auto *section = reinterpret_cast<const YINI::YiniSection *>(section_handle);
        if (index < 0 || index >= section->pairs.size())
            return 0;
        return safe_strncpy(buffer, section->pairs[index].key, buffer_size);
    }

    YINI_API const YiniValueHandle *yini_section_get_value_by_key(const YiniSectionHandle *section_handle,
                                                                  const char *key)
    {
        if (!section_handle || !key)
            return nullptr;
        auto *section = reinterpret_cast<const YINI::YiniSection *>(section_handle);
        auto it = std::find_if(section->pairs.begin(), section->pairs.end(),
                               [&](const YINI::YiniKeyValuePair &p) { return p.key == key; });
    ...
    YINI_API YiniType yini_value_get_type(const YiniValueHandle *value_handle)
    {
        ...
        if (std::holds_alternative<std::string>(value->data))
            return YINI_TYPE_STRING;
        if (std::holds_alternative<int>(value->data))
            return YINI_TYPE_INT;
        if (std::holds_alternative<double>(value->data))
            return YINI_TYPE_DOUBLE;
        if (std::holds_alternative<bool>(value->data))
            return YINI_TYPE_BOOL;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniArray>>(value->data))
            return YINI_TYPE_ARRAY;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniMap>>(value->data))
            return YINI_TYPE_MAP;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniDynaValue>>(value->data))
            return YINI_TYPE_DYNA;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniCoord>>(value->data))
            return YINI_TYPE_COORD;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniColor>>(value->data))
            return YINI_TYPE_COLOR;
        if (std::holds_alternative<std::unique_ptr<YINI::YiniPath>>(value->data))
            return YINI_TYPE_PATH;
        return YINI_TYPE_NONE;
    }
    ...
    YINI_API void yini_value_get_coord(const YiniValueHandle *value_handle, double *x, double *y, double *z,
                                       bool *is_3d)
    {
        if (!value_handle || !x || !y || !z || !is_3d)
            return;
        auto *value = reinterpret_cast<const YINI::YiniValue *>(value_handle);
        if (!std::holds_alternative<std::unique_ptr<YINI::YiniCoord>>(value->data))
            return;
    ...
    YINI_API void yini_value_get_color(const YiniValueHandle *value_handle, unsigned char *r, unsigned char *g,
                                       unsigned char *b)
    {
        if (!value_handle || !r || !g || !b)
            return;
        auto *value = reinterpret_cast<const YINI::YiniValue *>(value_handle);
        if (!std::holds_alternative<std::unique_ptr<YINI::YiniColor>>(value->data))
            return;
    ...
    YINI_API const YiniValueHandle *yini_array_get_value_by_index(const YiniValueHandle *value_handle, int index)
    {
        ...
        const auto &arr_ptr = std::get<std::unique_ptr<YINI::YiniArray>>(value->data);
        if (!arr_ptr || index < 0 || index >= arr_ptr->elements.size())
            return nullptr;
