...
namespace YINI
{
Lexer::Lexer(const std::string &input) : m_input(input), m_position(0), m_line(1), m_column(1)
{
}
...
Token Lexer::getNextToken()
{
    ...
    if (isdigit(current_char) ||
        (current_char == '-' && m_position + 1 < m_input.length() && isdigit(m_input[m_position + 1])))
    {
        return number();
    }
...
Token Lexer::identifier()
{
    ...
    while (m_position < m_input.length() &&
           (isalnum(m_input[m_position]) || m_input[m_position] == '_' || m_input[m_position] == '.'))
    {
        value += m_input[m_position];
        m_position++;
        m_column++;
